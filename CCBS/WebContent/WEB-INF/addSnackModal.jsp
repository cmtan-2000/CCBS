...
<body>
	...
	%>
	<form class="container p-5" style="background: white;" method="POST"
		action="/CCBS/company/fab/create">
		<c:choose>
			<c:when test="${empty fab}">
				<span class="float-end times-button"
					onclick="closeDialog('addFoodAndBeverageDialog')"
					style="cursor: pointer;"> <i class="fas fa-times"
					aria-hidden="true"></i></span>
			</c:when>
		</c:choose>

		<div class="d-flex">
			<label for="package">Package: </label> <select name="package"
				class="py-1 px-4 form-control"
				<c:choose>
		...
		<div class="row mt-3">
			<div class="col-6">
				<label>Name</label> <input name="name" value="${fab.getName()}"
			...
			<div class="col-6">
				...
				<div class="input-group mb-3 mt-3 ">
					...
					<input name="price" value="${fab.getPrice()}" class="form-control"
						required min="0" step="0.1" />
				</div>
				<label for="quantity">Quantity</label> <input name="quantity"
					value="${fab.getStock_quantity()}" class="mt-3 form-control"
					required min="0" step="1" />
			</div>
		</div>
		<input type="hidden" name="formType"
			value='${param.formType == "create" ? "create" : "edit"}'>
		<c:choose>
			<c:when test="${not empty fab}">
		...
		<button class="btn btn-primary font-weight-bold rounded-pill"
			type="submit" onclick="closeDialog('addFoodAndBeverageUpdate')">
			<c:choose>
				<c:when test="${not empty fab}">
					Update
				</c:when>
